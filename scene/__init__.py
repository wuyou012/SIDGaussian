...
from gaussian_utils.pose_utils import generate_random_poses_llff, generate_random_poses_360
...
class Scene2:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        if os.path.exists(os.path.join(args.source_path, "dense")):
            scene_info = sceneLoadTypeCallbacks["Colmap_dense"](args.source_path, args.images, args.eval, args.n_views)
        elif os.path.exists(os.path.join(args.source_path, "transforms_train.json")):
            print("Found transforms_train.json file, assuming Blender data set!")
            scene_info = sceneLoadTypeCallbacks["Blender"](args.source_path, args.white_background, args.eval, args.n_views)
        else:
