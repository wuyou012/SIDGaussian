...
from gaussian_utils.loss_utils import l1_loss, l1_loss_mask, ssim, get_vit_feature
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, testing_iterations, scene, renderFunc, renderArgs, metrics):
    if tb_writer:
...
def handle_network_gui(network_gui, iteration, opt, gaussians, pipe, background, dataset):
...
def training(dataset, opt, pipe, args, metrics):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        handle_network_gui(network_gui, iteration, opt, gaussians, pipe, background, dataset)
        ...
        flag_pseudo = False
        if iteration % args.sample_pseudo_interval == 0 and args.start_sample_pseudo < iteration < args.end_sample_pseudo:
            if not pseudo_stack:
        ...
        with torch.no_grad():
            ...
            if iteration == opt.iterations:
                progress_bar.close()
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, args.test_iterations, scene, render, (pipe, background), metrics)
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    gaussians.densify_and_prune(opt.densify_grad_threshold, opt.prune_threshold, scene.cameras_extent, None, iteration)
            if iteration < opt.iterations:
